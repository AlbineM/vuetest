...
onMounted(() => {
    ...
    today.value = today.value.getDay()
})
                    ...
                    <div class="py-2 inline-block min-w-full sm:px-6 lg:px-8">
                        <div class="overflow-hidden">
                            <table class="min-w-full text-center">
                                <caption class="text-xs font-semibold text-gray-900">
                                    Opening times on {{ information?.hours_type }} days
                                </caption>
                                <thead class="border-b bg-white">
                                    <tr class="font-semibold">
                                        <th scope="col" class="text-xs text-gray-900 px-6 py-2 text-left">
                                            Day
                                        </th>
                                        <th scope="col" class="text-xs text-gray-900 px-6 py-2">
                                            Opens at
                                        </th>
                                        <th scope="col" class="text-xs text-gray-900 px-6 py-2">
                                            Closes at
                                ...
                                <tbody v-for="details, index in information?.open" :key="index">
                                    <tr class="bg-white">
                                        <td class="px-6 py-2 whitespace-nowrap text-left text-sm font-medium text-gray-900">{{ DAYS[details?.day] }} <span class="font-semibold text-teal-600 italic ml-2" v-if="today === index">Today</span></td>
